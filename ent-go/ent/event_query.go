...
func (eq *EventQuery) QueryGuest() *UserQuery {
	query := (&UserClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (eq *EventQuery) QueryTicket() *TicketQuery {
...
func (eq *EventQuery) Clone() *EventQuery {
	...
	return &EventQuery{
		...
		withGuest:    eq.withGuest.Clone(),
		withTicket:   eq.withTicket.Clone(),
...
func (eq *EventQuery) WithGuest(opts ...func(*UserQuery)) *EventQuery {
	query := (&UserClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EventQuery) WithTicket(opts ...func(*TicketQuery)) *EventQuery {
	query := (&TicketClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := eq.withGuest; query != nil {
		if err := eq.loadGuest(ctx, query, nodes,
			func(n *Event) { n.Edges.Guest = []*User{} },
			func(n *Event, e *User) { n.Edges.Guest = append(n.Edges.Guest, e) }); err != nil {
			return nil, err
		}
	}
	if query := eq.withTicket; query != nil {
		if err := eq.loadTicket(ctx, query, nodes,
			func(n *Event) { n.Edges.Ticket = []*Ticket{} },
			func(n *Event, e *Ticket) { n.Edges.Ticket = append(n.Edges.Ticket, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EventQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Event, init func(*Event), assign func(*Event, *User)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "organizer_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (eq *EventQuery) loadLocation(ctx context.Context, query *LocationQuery, nodes []*Event, init func(*Event), assign func(*Event, *Location)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
...
func (eq *EventQuery) loadGuest(ctx context.Context, query *UserQuery, nodes []*Event, init func(*Event), assign func(*Event, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(event.GuestColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.event_guest
		if fk == nil {
			return fmt.Errorf(`foreign-key "event_guest" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "event_guest" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (eq *EventQuery) loadTicket(ctx context.Context, query *TicketQuery, nodes []*Event, init func(*Event), assign func(*Event, *Ticket)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Event)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Ticket(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(event.TicketColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.event_ticket
		if fk == nil {
			return fmt.Errorf(`foreign-key "event_ticket" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "event_ticket" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (eq *EventQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := eq.ctx.Fields; len(fields) > 0 {
		...
		if eq.withUser != nil {
			_spec.Node.AddColumnOnce(event.FieldOrganizerID)
		}
		if eq.withLocation != nil {
			_spec.Node.AddColumnOnce(event.FieldLocationID)
		}
	}
