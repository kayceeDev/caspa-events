...
func (lq *LocationQuery) QueryEvent() *EventQuery {
	query := (&EventClient{config: lq.config}).Query()
...
func (lq *LocationQuery) WithEvent(opts ...func(*EventQuery)) *LocationQuery {
	query := (&EventClient{config: lq.config}).Query()
...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := lq.withEvent; query != nil {
		if err := lq.loadEvent(ctx, query, nodes,
			func(n *Location) { n.Edges.Event = []*Event{} },
			func(n *Location, e *Event) { n.Edges.Event = append(n.Edges.Event, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LocationQuery) loadEvent(ctx context.Context, query *EventQuery, nodes []*Location, init func(*Location), assign func(*Location, *Event)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(event.FieldLocationID)
	}
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.EventColumn), fks...))
	}))
