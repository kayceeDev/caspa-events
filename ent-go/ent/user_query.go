...
func (uq *UserQuery) QueryEvent() *EventQuery {
...
func (uq *UserQuery) WithEvent(opts ...func(*EventQuery)) *UserQuery {
	query := (&EventClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withEvent; query != nil {
		if err := uq.loadEvent(ctx, query, nodes,
			func(n *User) { n.Edges.Event = []*Event{} },
			func(n *User, e *Event) { n.Edges.Event = append(n.Edges.Event, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadEvent(ctx context.Context, query *EventQuery, nodes []*User, init func(*User), assign func(*User, *Event)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(event.FieldOrganizerID)
	}
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.EventColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organizer_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
